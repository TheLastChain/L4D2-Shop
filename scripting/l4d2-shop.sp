...
public OnPluginStart()
{
	...
	g_ChargerKilled = CreateConVar("sm_chargerkilled", "25", "Giving money for killing a charger");
	g_SmokerKilled = CreateConVar("sm_smokerkilled", "25", "Giving money for killing a smoker");
	g_HunterKilled = CreateConVar("sm_hunterkilled", "25", "Giving money for killing a hunter");
	g_JockeyKilled = CreateConVar("sm_jockeykilled", "25", "Giving money for killing a jockey");
	g_SpitterKilled = CreateConVar("sm_spitterkilled", "25", "Giving money for killing a spitter");
	g_TankKilled = CreateConVar("sm_tankkilled", "100", "Giving money for killing a tank");
	g_WitchKilled = CreateConVar("sm_witchkilled", "50", "Giving money for killing a witch");
	g_ZombieKilled = CreateConVar("sm_zombiekilled", "1", "Giving money for killing a zombie");
}
/*///////////////////////////////////////////////////////////////
                       _       
                      | |      
...
public Action GiveMoney(int client, int args) {
	...
	PrintToChat(client, "[SM] You gave out %i to a %s", money, name);
...
public witch_killed(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	PrintToChat(client, "[SM] You got %i for killing witch", WitchKilled);
}
public player_death(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (attacker != client) {
		...
		{
			PrintToChat(attacker, "[SM] You got %i credits for killing the Boomer!", BoomerKilled);
			g_iCredits[attacker] += BoomerKilled;
		}
		if (strcmp(victimname, "smoker", false) == 0)
		{
			PrintToChat(attacker, "[SM] You got %i credits for killing the Smoker!", SmokerKilled);
			g_iCredits[attacker] += SmokerKilled;
		}
		if (strcmp(victimname, "charger", false) == 0)
		{
			PrintToChat(attacker, "[SM] You got %i credits for killing the Charger!", ChargerKilled);
			g_iCredits[attacker] += ChargerKilled;
		}
		if (strcmp(victimname, "hunter", false) == 0)
		{
			PrintToChat(attacker, "[SM] You got %i credits for killing the Hunter!", HunterKilled);
			g_iCredits[attacker] += HunterKilled;
		}
		if (strcmp(victimname, "jockey", false) == 0)
		{
			PrintToChat(attacker, "[SM] You got %i credits for killing the Jockey!", JockeyKilled);
			g_iCredits[attacker] += JockeyKilled;
		}
		if (strcmp(victimname, "spitter", false) == 0)
		{
			PrintToChat(attacker, "[SM] You got %i credits for killing the Spitter!", SpitterKilled);
...
public tank_killed(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	PrintToChat(client, "[SM] You got %i credits for killing Tank!", TankKilled);
}
...
public Action:HinT(client, args)
{
	...
	SetMenuTitle(menu, "[Shop] Your money: %d", g_iCredits[client]);
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if (StrEqual(item, "option1"))
			{
				...
				AddMenuItem(menu, "option14", "AWP - 150 credits");
				AddMenuItem(menu, "option15", "Scout - 120 credits");
				AddMenuItem(menu, "option16", "SG552 - 300 credits");
				AddMenuItem(menu, "option17", "MP5 - 125 credits");
				AddMenuItem(menu, "option18", "Grenade Launcher - 250 credits");
				SetMenuExitButton(menu, true);
			...
			if (StrEqual(item, "option2"))
			{
				...
				AddMenuItem(menu, "option6", "Pitchfork - 150 credits");
				AddMenuItem(menu, "option7", "Shovel - 150 credits");
				SetMenuExitButton(menu, true);
			...
			if (StrEqual(item, "option3"))
			{
				...
				AddMenuItem(menu, "option1", "Defibrilator - 100 credits");
...
public Action:Pay(client, args) {
	...
	if (args != 2) {
		ReplyToCommand(client, "[SM] usage: !pay <name> <money>");
	}
	g_iCredits[target] += paymoney;
	g_iCredits[client] -= paymoney;
	PrintToChat(target, "[SM] %s gave you %i credits", client, paymoney);
	PrintToChat(client, "[SM] You gave: %i credits", paymoney)
	return Plugin_Handled;
}
	...
	if (action == MenuAction_Select) {
		if (StrEqual(Item, "option1"))
		{
			if (g_iCredits[client] >= 75) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 75 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option2"))
		{
			if (g_iCredits[client] >= 200) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 200 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option3"))
		{
		...
		if (StrEqual(Item, "option5"))
		{
			if (g_iCredits[client] >= 150) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 150 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option6"))
		{
			if (g_iCredits[client] >= 130) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 130 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option7"))
		{
			if (g_iCredits[client] >= 125) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 125 but you have %i credits", g_iCredits[client]);
			}
		}
		...
		if (StrEqual(Item, "option9"))
		{
			if (g_iCredits[client] >= 250) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 250 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option10"))
		{
			if (g_iCredits[client] >= 255) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 255 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option11"))
		{
			if (g_iCredits[client] >= 500) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 500 but you have %i credits", g_iCredits[client]);
			}
		}
		...
		if (StrEqual(Item, "option15"))
		{
			if (g_iCredits[client] >= 120) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 120 but you have %i credits", g_iCredits[client]);
			}
		}
	...
	if (action == MenuAction_Select) {
		if (StrEqual(Item, "option1"))
		{
			if (g_iCredits[client] >= 100) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 100 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option2"))
		{
			if (g_iCredits[client] >= 50) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 50 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option3"))
		{
			if (g_iCredits[client] >= 30) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 30 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option3"))
		{
		...
		if (StrEqual(Item, "option4"))
		{
			if (g_iCredits[client] >= 25) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 25 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option5"))
		{
			if (g_iCredits[client] >= 300) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 300 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option6"))
		{
		...
		if (StrEqual(Item, "option7"))
		{
			if (g_iCredits[client] >= 45) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 45 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option8"))
		{
			if (g_iCredits[client] >= 55) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 55 but you have %i credits", g_iCredits[client]);
			}
		}
		if (StrEqual(Item, "option9"))
		{
			if (g_iCredits[client] >= 60) {
				...
				PrintToChat(client, "[SM] You don't have enough credit! You need 60 but you have %i credits", g_iCredits[client]);
			}
		}
	}
	SetCommandFlags("give", flagszspawn | FCVAR_CHEAT);
